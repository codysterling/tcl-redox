...
proc mdstep { } {
    ...
    if { ($istep == $stepb) } { dyn1 configure temperature = $tempb ensemble=NVT; puts "changing temp to $tempb" }
    if { ($istep == $stepc) } { dyn1 configure temperature = $tempc ensemble=NVT; puts "changing temp to $tempc"}
    if { ($istep == $stepd) } { dyn1 configure temperature = $tempd ensemble=NVT; puts "changing temp to $tempd" }
    if { ($istep == $stepe) } { dyn1 configure temperature = $tempe ensemble=NVT; puts "changing temp to $tempe" }
